...
There are three internal constructors. The first assigns zeros to the vector field. The second assigns a raw array of reals of the appropriate dimension. And the third assigns a mutable array of reals of the appropriate dimension.
...
```
function getindex(y::PhysicalVector, index::Integer)::PhysicalScalar
function setindex!(y::PhysicalVector, scalar::PhysicalScalar, index::Integer)
```

While the following methods are to be used to retrieve and assign a `PhysicalVector` from/to an `ArrayOfPhysicalVectors.`

```
function getindex(y::ArrayOfPhysicalVectors, index::Integer)::PhysicalVector
function setindex!(y::ArrayOfPhysicalVectors, vector::PhysicalVector, index::Integer)
```
...
```
function copy(y::PhysicalVector)::PhysicalVector
function copy(y::ArrayOfPhysicalVectors)::ArrayOfPhysicalVectors
```
...
```
function toString(y::PhysicalVector)::String
```

Conversion to the raw array of reals held by the vector is provided for by
```
function toVector(y::PhysicalVector)::Vector{<:Real}
```
...
```
returns a dimensionless vector with Euclidean length 1 pointing in the direction of vector `y.`
